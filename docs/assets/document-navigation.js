(function () {
  ...
  const CURRENT_CONTEXT = Object.freeze({
    ...
    "architecture/jira_data_relationship_map.html": [
      "두 DB를 함께 보는 문서",
      "Operational State DB 개정 3과 이미 구현된 Knowledge DB 개정 1을 같은 화면에서 비교합니다. 숫자 크기를 서로 비교하면 안 됩니다.",
    ],
  ...
  function pageUsesVersionTerms() {
    ...
    return /\bv\d+(?:\.\d+)?\b/i.test(text)
      || /semantic_v2|schema_version|user_version|revision/i.test(text);
  }

  function historicalContext(path) {
    if (!/_v[12]_baseline\.html$/i.test(path)) return null;
    return [
      "과거 설계 보관본 · 현재 구현 기준 아님",
      "이 문서는 설계 변화 과정을 보존하기 위한 기록입니다. 현재 기준은 문서 Hub의 '현재 운영 규칙' 문서를 따르세요.",
    ];
  }
  ...
  function buildVersionHelp() {
    ...
    lead.textContent = "핵심: 숫자가 더 크다고 다른 종류의 문서보다 더 최신이라는 뜻이 아닙니다. 항상 '무엇의 버전인지'와 '현재/과거 상태'를 먼저 보세요.";
    ...
    grid.append(
      ...
      versionItem("semantic_v2", "문서 버전이 아니라 <strong>Jira 내용이 의미상 바뀌었는지 판별하는 hash profile 이름</strong>입니다."),
